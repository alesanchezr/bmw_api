...
class JsonPDO{
    ...
    function parseFile($fileName, $defaultContent){
        ...
        else
        {
            ...
            if(empty($jsonContent)) $this->throwError('Unable to get file "'.$fileName." or the content is empty");
            if($jsonContent == '[]' || $jsonContent == '{}') return [];
            $dataContent = (array) json_decode($jsonContent);
            if($dataContent == false) $this->throwError('The file "'.$fileName.'" has invalid syntax: '.$jsonContent);
